...
import { DataTable, SelectOptionObject } from '@/src/index'
...
const columns = {
  ...
  role: {
    type: 'select',
    selectValue: (option: SelectOptionObject) => option.text,
    selectOptions: [
      { text: 'admin' },
      { text: 'user'  },
      { text: 'guest' }
...
async function selectOption(n: number, text: string, rows: string[]) {
  ...
  const inputs = select.querySelector('div.inputs') || fail('no input')
