...
const RangeExample = () =>
  ...
  >
    ...
    <p>
      Inside the scope of the <code>RangeMinMax</code> element, any children can
      access the context using the <code>useRangeMinMax</code> hook.  Immediate
      children of the <code>RangeMinMax</code> component can also be functions
      which will receive an object containing all the context properties as
      an argument.
    </p>
    <p>
      In this example we first render the <code>RangeMinMaxOutput</code> and{' '}
      <code>RangeMinMaxSlider</code>{' '} sub-components (lines 10 and 11).
      Then we include several copies of the custom <code>SetMin</code> (lines 14-16)
      and <code>SetMax</code> (lines 19-21) components
      which use the <code>useRangeMinMax</code> hook to access the{' '}
